...
-- USS Reasoner 82 by James J. Jackson

GT = {};
dofile(current_mod_path..'/Database/scripts/Reasoner_RunwaysAndRoutes.lua') -- the runways and taxiway lua file
GT_t.ws = 0;

set_recursive_metatable(GT, GT_t.generic_ship) -- all ships have this

GT.visual = {}
GT.visual.shape = "Reasoner_dcs"  -- name of LODs lua
...
GT_t.LN_t.ship_mk42_127mm_AIR.PL[1] = {
							shell_name = {"cwv_rea_mk42_127mm_AIR"},
...
GT.WS[ws].reference_angle_Z = math.rad(0);
GT.WS[ws].LN[1].depends_on_unit = Reasoner_TRACKERS
...
ws = GT_t.inc_ws()
local first_Reasoner_tracker_id = ws
GT.WS[ws] = {
	area = 'SuperStructure',
...
local Reasoner_TRACKERS = {{{'self', ws}}}

for i=2,13 do 
    ws = GT_t.inc_ws()
    GT.WS[ws] = {}
    set_recursive_metatable(GT.WS[ws], GT.WS[first_Reasoner_tracker_id])
    table.insert(Reasoner_TRACKERS, {{'self', ws}})
end
-------------------------------------------------------------------------------------------------
--   Torpedos
-------------------------------------------------------------------------------------------------
...
GT.Name = "USS Reasoner"; -- folder name for Liveries
GT.DisplayName = _("[CWV] USS Reasoner (FF-1063)"); -- name in game in ME and on the tape at the bottom
GT.DisplayNameShort = _("Knox class (RIM-7)"); -- Label name
GT.Rate = 3000.000000;
